...
def main(_):
	...
	h_pool1 = max_pool_nx1(h_conv1, MAX_POOLING1)


	W_conv2 = weight_variable([second, 1, CHANNEL_1, CHANNEL_2])
	b_conv2 = bias_variable([CHANNEL_2])

	h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2)
	h_pool2 = max_pool_nx1(h_conv2, MAX_POOLING2)


	h_pool2_flat = tf.reshape(h_pool2, [-1, int(1024 / (MAX_POOLING1 * MAX_POOLING2) * CHANNEL_2)])


	W_fc1 = weight_variable([int(1024 / (MAX_POOLING1 * MAX_POOLING2) * CHANNEL_2), fc])
	...
	#read & parse data
		...
		if i % 100 == 0:
			...
			for j in range(int(VALIDATION_RECORDS / 100)):
				...
				training_accuracy += int(sess.run(accuracy, feed_dict={x: temps, y_: templ, keep_prob: 1.0}) * 100)
			training_accuracy = training_accuracy / VALIDATION_RECORDS
			#q.append(training_accuracy)
			...
			print 'Step {}: {}'.format(i, training_accuracy)
